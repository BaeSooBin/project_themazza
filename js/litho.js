;(function($){
    ...
    var litho = {      
        ...
        section2Fn:function(){
          //스크롤시 섹션2가 노출되면(로딩시)
          //3칸이 좌측에서 우측으로 차례로 페이드 인효과
          //첫번째칸 페이드아웃(css opacity:0) 페이드인(opacity:1) time:0.6s
          ...
          var t = 0;  //스크롤 이벤트 토글 변수
            ...
            $(window).scroll(function(){
              //상단 섹션1에서 스크롤탑값이 400픽셀 아래로 내려오면 섹션2의 페럴럭스 애니메이션 수행
              if( $(this).scrollTop() == 0 ){
                t=0;
        ...
        section4Fn:function(){
            ...
            var y         = 0;
            var t         = 0;
            var $sec4Top = $('#section4').offset().top; //섹션4의 탑값
        ...
        section9Fn:function(){
          ...
          var $leftW = $('#section9 .content-wrap ul .left').innerWidth();
          //$ul 높이 = $left 박스의 너비값 * 높이 비율 1.38568306076
          var $ulH = $leftW * 1.385683076;
          var $h3 = $('#section9 .right-wrap h3'); 
          var $p  = $('#section9 .right-wrap p'); 
          var $h6 = $('#section9 .right-wrap h6'); 
          var h3Sz =  $leftW * 0.069477146;
          var pSz  =  $leftW * 0.057897622;
          var h6Sz =  $leftW * 0.046318097;


              //1. 반응형 함수
              function resizeFn(){
                $leftW = $('#section9 .content-wrap ul .left').innerWidth();
                $ulH = $leftW * 1.385683076;
                h3Sz =  $leftW * 0.069477146;
                pSz  =  $leftW * 0.057897622;
                h6Sz =  $leftW * 0.046318097;

                $ul.css({ height : $ulH });
                $h3.css({ fontSize : h3Sz });
                $p.css({ fontSize : pSz });
                $h6.css({ fontSize : h6Sz });
              }
        ...
        section10Fn:function(){
          ...
          var $section3 = $('#section3');
          var $sec10Top = $('#section10').offset().top-400;
          var t = 0;
        ...
        footerFn:function(){
          //폼을 이용한 메일주소를 php(서버사이드 스크립트언어) 비동기전송
          //ajax
          ...
          var $inputWrap  = $('.input-wrap');
            ...
            $submitBtn.on({
              click:function(e){
                e.preventDefault();  //전송(submit)버튼 기능을 삭제
                var $frmEmail = $('#frmEmail').val();  //폼 입력상자 value
                var $frmCode  = $('#frmcode').val();   //폼 입력상자 value

                //유효성 검사
                if( $frmEmail == '' ){
                  // alert('메일 주소를 입력하세요! ');
                  $inputWrap.addClass('addAjax');
                  $('#frmEmail').focus();  //입력대기상태로 이동 포커스
                  ...
                  $.ajax({
                    ...
                    success:function(result){
                      ...
                      function msgFn(){
                        $('#frmEmail').val('');
                        // $response.children().remove();  response 지우기
                        // $response.empty();  response 지우기
                        $response.html('');
                        $inputWrap.removeClass('addAjax');
        ...
        smoothScrollFn:function(){
          ...
          var t = 0;
            ...
            $smoothBtn.on({
              click:function(e){
                ...
                var url = $(this).attr('href');
                  $('html,body').stop().animate({scrollTop : $(url).offset().top }, 800, 'easeInOutSine');
              }
            });


            //창너비가 1200 초과이면 모달창 보이고 아니면 안보인다.
            function resizeFn(){
            ...
            //최상단에서 100px 이상 아래로 스크롤하면 goTop 메뉴가 보이고
            //맨위로 올라가면 안보인다.
            $(window).scroll(function(){
        ...
        demoModalFn:function(){
          //모달창 사용 html overflow:hidden 데모사이트를 우측에서 부드럽게
          //슬라이딩방식의 스크립트 애니메이션 제작

          //.modal-btn 버튼을 클릭하면 html 스크롤을 삭제하고 addClass('addModal');
          //#header 헤더를 위로 올려서 숨긴다. addClass('addHide');
          //#modalDemo 데모모달창을 우측에서 좌측으로 부드럽게 슬라이딩 애니메이션 효과
          var $html = $('html');
          var $header = $('#header');
          var $modalDemo = $('#modalDemo');
